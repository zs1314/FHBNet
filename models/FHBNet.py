...
from models.MSCCA_Block import CrissCrossAttention
from models.MSCCA_Block import PSAModule
from models.FHB_BRA.FHB_BRA_Block import FHB_bra
...
class MSCCA_block2(nn.Module):
    def __init__(self, in_channels, internal_channels, out_channels, drop_path):
        ...
        self.cc_att = CrissCrossAttention(out_channels)
...
class MSCCA_block1(nn.Module):
    ...
    def __init__(self, in_channels, dw_channels, block_lk_size, small_kernel, drop_path, small_kernel_merged=False):
        ...
        self.epsa_block = PSAModule(inplans=in_channels, planes=in_channels)
    ...
    def forward(self, x):
        ...
        out = self.pw2(out)
        out = self.epsa_block(out)
        return x + self.drop_path(out)


class MSCCAStage(nn.Module):
    ...
    def __init__(self, channels, num_blocks, stage_lk_size, drop_path,
        ...
        for i in range(num_blocks):
            ...
            block1 = MSCCA_block1(in_channels=channels, dw_channels=int(channels * dw_ratio), block_lk_size=stage_lk_size,
                                     small_kernel=small_kernel, drop_path=block_drop_path, small_kernel_merged=small_kernel_merged)
            block2 = MSCCA_block2(in_channels=channels, internal_channels=int(channels * ffn_ratio), out_channels=channels,
...
class FHBNet(nn.Module):
    ...
    def __init__(self, large_kernel_sizes, layers, channels, drop_path_rate, small_kernel,
        ...
        self.FHB_Bra = FHB_bra()
        self.FHB_bra_block = self.FHB_Bra.ret_stage()
        for stage_idx in range(self.num_stages):
            layer = MSCCAStage(channels=channels[stage_idx], num_blocks=layers[stage_idx],
    ...
    def forward_features(self, x):
        ...
        if self.out_indices is None:
            ...
            for stage_idx in range(self.num_stages):
                ...
                FHB_bra_block = self.FHB_bra_block[stage_idx]
                x = FHB_bra_block(x)
...
def create_FHBNet(drop_path_rate=0.3, num_classes=1000, use_checkpoint=True, small_kernel_merged=False):
    return FHBNet(large_kernel_sizes=[31,29,27,13], layers=[2,2,18,2], channels=[128,256,512,1024],
                    drop_path_rate=drop_path_rate, small_kernel=5, num_classes=num_classes, use_checkpoint=use_checkpoint,
                    small_kernel_merged=small_kernel_merged)


if __name__ == '__main__':
    model = create_FHBNet(small_kernel_merged=False,num_classes=1).to('cuda:0')
    model.eval()
    print(model)
